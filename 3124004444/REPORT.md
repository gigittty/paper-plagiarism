...
4. **次线性词频缩放**：以 `tf^0.88` 代替原始词频，抑制高频常用词对相似度的主导作用，
   使度量对「插字 / 改字」这类字词级改写更敏感，避免把仅有字词级改写的抄袭版误判为几乎完全一致。
5. **异常不外抛**：所有可预期错误转换为自定义异常并在入口统一捕获，返回受控退出码，
...
| 样例 | 重复率 | 说明 |
| --- | ---: | --- |
| orig_0.8_add（在原文中插字） | 98.05 | 高相似度，符合预期 |
